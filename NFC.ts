...
namespace NFC {
    ...
    function passwdCheck(blockN: number, id: Buffer, st: Buffer): boolean {
        ...
        buf = [0x00, 0x00, 0xFF, 0x0F, 0xF1, 0xD4, 0x40, 0x01, 0x60, 0x07, 0xFF,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x66, 0xDC, 0x89, 0xAB, 0xC2, 0x00];
    ...
    function writeblock(blockN: number, data: Buffer): void {
        ...
        serial.writeLine(uId.toString());
        serial.writeLine(passwdBuf.toString());
    ...
    export function readDataByte(block: number, byteN: number): number {
        let ret = -1;
        readDataBlock(block);
        ret = blockData[byteN - 1];
        return ret;
    }
    //% weight=4
